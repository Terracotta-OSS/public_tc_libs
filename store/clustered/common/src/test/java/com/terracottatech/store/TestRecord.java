...
public class TestRecord<K extends Comparable<K>> implements InternalRecord<K> {
  ...
  public TestRecord(RecordData<K> data) {
    this(data.getMsn(), data.getKey(), data.getCells());
  }

  public TestRecord(long msn, K key, Iterable<Cell<?>> cells) {
    this.msn = msn;
    this.key = key;
