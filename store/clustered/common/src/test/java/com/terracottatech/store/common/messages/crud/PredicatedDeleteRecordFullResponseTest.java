...
public class PredicatedDeleteRecordFullResponseTest {
  @Test
  public void getValuesPassedToConstructor() {
    PredicatedDeleteRecordFullResponse<String> response = new PredicatedDeleteRecordFullResponse<>("foo", 1L, Collections.emptyList());
    Assert.assertEquals(DatasetEntityResponseType.PREDICATED_DELETE_RECORD_FULL_RESPONSE, response.getType());
    assertEquals(1L, response.getDeleted().getMsn());
  }
  ...
  public void recordWithCells() {
    ...
    PredicatedDeleteRecordFullResponse<String> response = new PredicatedDeleteRecordFullResponse<>("foo", 2L, Arrays.asList(cell1, cell2));
    assertAllCellsEqual(response.getDeleted().getCells(), Arrays.asList(cell1, cell2));
  }
}
