...
public class PredicatedDeleteRecordMessageTest {
  private final UUID stableClientId = UUID.randomUUID();

  @Test
  public void hasPredicatedDeleteRecordMessageType() {
    IntrinsicPredicate<Record<?>> recordPredicate = AlwaysTrue.alwaysTrue();
    PredicatedDeleteRecordMessage<String> message = new PredicatedDeleteRecordMessage<>(stableClientId, "key", recordPredicate, true);
    assertEquals(DatasetOperationMessageType.PREDICATED_DELETE_RECORD_MESSAGE, message.getType());
    assertEquals(ChangeType.DELETION, message.getChangeType());
