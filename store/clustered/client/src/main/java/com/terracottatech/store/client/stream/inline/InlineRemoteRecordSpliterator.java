 ...
 */
public class InlineRemoteRecordSpliterator<K extends Comparable<K>>
    extends AbstractInlineRemoteSpliterator<K, Record<K>> {

  /**
   * Creates a new {@code InlineRemoteRecordSpliterator} against the remote {@link com.terracottatech.store.Dataset Dataset}.
   *
   ...
   */
  public InlineRemoteRecordSpliterator(MessageSender messageSender, UUID streamId, RootStreamDescriptor descriptor,
  ...
  public boolean tryAdvance(Consumer<? super Record<K>> action) {
    ...
    return this.<K>consume((fetchApplyResponse) -> {
      RecordData<K> recordData = fetchApplyResponse.getElement().getRecordData();
      action.accept(new RecordImpl<>(recordData));
    });
  }
}
