...
/**
 * Verifies the closure semantics of operations on a {@link RemoteReferenceStream}.
 */
public class RemoteReferenceStreamClosureTest<T> extends AbstractStreamClosureTest<T> {
  ...
  protected Stream<T> getStream() {
    ...
    // Force into non-Record Stream; direct access avoids intrinsic check
    rootStream.appendPortablePipelineOperation(
        MAP.newInstance((Function<Record<String>, Optional<Long>>)(r) -> r.get(Animals.Schema.OBSERVATIONS)));
    return new RemoteReferenceStream<>(rootStream);
  }
}
