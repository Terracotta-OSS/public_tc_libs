...
public class DeferredMutableRecordStreamClosureTest extends AbstractStreamClosureTest<Record<String>> {
  ...
  protected Stream<Record<String>> getStream() {
    ...
    when(a.spliterator()).thenReturn(Stream.<Record<String>>generate(() -> mock(Record.class)).limit(10).spliterator());
    when(b.spliterator()).thenReturn(Stream.<Record<String>>generate(() -> mock(Record.class)).limit(10).spliterator());
    ...
    return new DeferredMutableRecordStream<>(
            new ShardedRecordStream<>(shards, null, 0, source),
            new ShardedMutableRecordStream<>(shards, 0, source));
  }
  ...
  public void testSorted0Arg() throws Exception {
    try {
      ...
      assertThat(e.getMessage(), containsString("sorted() is not supported"));
    }
  }
