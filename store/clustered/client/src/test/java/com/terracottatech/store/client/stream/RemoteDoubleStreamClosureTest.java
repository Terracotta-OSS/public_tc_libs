...
/**
 * Verifies the closure semantics of operations on a {@link RemoteDoubleStream}.
 */
public class RemoteDoubleStreamClosureTest extends AbstractDoubleStreamClosureTest {
  ...
  protected DoubleStream getStream() {
    ...
    // Force into DoubleStream; direct access avoids intrinsic check
    rootStream.appendPortablePipelineOperation(
        MAP_TO_DOUBLE.newInstance((ToDoubleFunction<Record<String>>)(r) -> r.get(Animals.Schema.OBSERVATIONS).orElse(0L).doubleValue()));
    return new RemoteDoubleStream<>(rootStream);
  }
}
