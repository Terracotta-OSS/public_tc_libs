 ...
 * Test for TDB-2807
 *
 * Test to reproduce memory leak by portable mutative streams whose entries never get removed from
 * streamMap in DatasetActiveEnitity resulting OOM.
 *
 * @author Eugene Sebastian
 */
@Ignore
public class MemoryLeakPortableStreamIT extends BaseSystemTest {
      ...
      futures.add(service.submit(() -> {
        for (; ; ) {
          ...
          writerReader.records()
              .filter(CELL_1.value().is(recordId))
              .mutate(write(CELL_1).intResultOf(CELL_1.intValueOrFail().increment()));
          if (recordId % 1000 == 0) {
