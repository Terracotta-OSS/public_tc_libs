...
class TransactionalRecordStream<K extends Comparable<K>> implements RecordStream<K> {

  private final Stream<Record<K>> delegate;

  TransactionalRecordStream(Stream<Record<K>> delegate) {
  ...
  protected RecordStream<K> wrap(Stream<Record<K>> stream) {
    return new TransactionalRecordStream<>(stream);
  }
}
