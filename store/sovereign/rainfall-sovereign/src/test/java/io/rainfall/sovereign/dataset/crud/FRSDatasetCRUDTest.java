 ...
 */
package io.rainfall.sovereign.dataset.crud;
 ...
 * @author cschanck
 **/
public class FRSDatasetCRUDTest extends AbstractDatasetTest {

  private SovereignConfig<Long> config;
  private SovereignFRSStorage store;
  ...
  public void before() throws IOException, ExecutionException, InterruptedException {
    ...
    this.store = new SovereignFRSStorage(new PersistenceRoot(tmp.newFolder(), PersistenceRoot.Mode
  ...
  @Test
  public void testFRSCRUD() throws SyntaxException {
    config.setDescription("FRS CRUD");
    runAddReplaceGetDelete(config, "frs", .3, .4, .3);
  }
