...
public class Get<K extends Comparable<K>, T> extends SovereignOperation<K, T> {
  ...
  public void exec(final StatisticsHolder statisticsHolder,
    ...
    SovereignConfig<K> sovConfig = (SovereignConfig) configurations.get(SovereignConfig.class);
    ...
    for (final SovereignDatasetImpl<K> ds : sovConfig) {
      if (!ds.isDisposed()) {
        ...
        try {
          Record<K> prior = ds.get(k);
          long end = getTimeInNs();
          if (prior == null) {
            statisticsHolder.record(cName, (end - start), SovereignResult.GETMISS);
          } else {
            statisticsHolder.record(cName, (end - start), SovereignResult.GET);
          }
        } catch (Throwable e) {
