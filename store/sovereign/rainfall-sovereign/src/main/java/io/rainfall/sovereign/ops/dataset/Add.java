...
public class Add<K extends Comparable<K>, T> extends SovereignOperation<K, T> {

  private String cName = getClass().getSimpleName();

  public Add() {
    super(SovereignResult.ADD, SovereignResult.ADDMISS);
  }
  ...
  public void exec(final StatisticsHolder statisticsHolder,
    ...
    for (final SovereignDatasetImpl<K> ds : sovConfig) {
      if (!ds.isDisposed()) {
        Cell<?> l = Cell.cell("counter", valueGenerator.generate(next));
        ...
        try {
          ...
          if (prior == null) {
            ...
            statisticsHolder.record(cName, (end - start), SovereignResult.ADDMISS);
          }
        } catch (Throwable e) {
