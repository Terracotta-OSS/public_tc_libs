...
public class BtreeDeleteOperation extends SovereignOperation<Long, Long> {

  public BtreeDeleteOperation() {
  ...
  public void exec(StatisticsHolder stats, Map<Class<? extends Configuration>, Configuration> configurations,
    ...
    try {
      ...
      if (removed == null) {
        stats.record(getClass().getSimpleName(), (end - start), SovereignResult.DELETEMISS);
      } else {
        stats.record(getClass().getSimpleName(), (end - start), SovereignResult.DELETE);
      }
    } catch (Throwable e) {
      long end = getTimeInNs();
