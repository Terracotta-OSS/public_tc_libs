 ...
 * @author mscott
 */
public class MultiVersionOffheapDatasetApiTest extends AbstractDatasetApiTest {

  @Override
  protected <K extends Comparable<K>> SovereignDataset<K> createDataset(SovereignDataSetConfig<K, ?> config) {
    return new SovereignBuilder<>(config.resourceSize(OFFHEAP, 16 * RESOURCE_SIZE).versionLimit(2)).build();
  }
