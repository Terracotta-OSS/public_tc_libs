 ...
 * @author cschanck
 */
public class MultiVersionHybridDatasetApiTest extends AbstractDatasetApiTest {
  @ClassRule
  ...
  protected <K extends Comparable<K>> SovereignDataset<K> createDataset(SovereignDataSetConfig<K, ?> config) {
    return new SovereignBuilder<>(config.resourceSize(OFFHEAP, 16 * RESOURCE_SIZE).concurrency(4).versionLimit(2).storage(storage)).build();
  }
}
