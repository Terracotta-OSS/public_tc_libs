 ...
 * @author cschanck
 */
public class MultiVersionFRSDatasetApiTest extends AbstractDatasetApiTest {
  ...
  public void createStorage() throws Exception {
    if (storage == null) {
      storage = new SovereignFRSStorage(new PersistenceRoot(TEMP_FOLDER.newFolder(),
  ...
  protected <K extends Comparable<K>> SovereignDataset<K> createDataset(SovereignDataSetConfig<K, ?> config) {
    return new SovereignBuilder<>(config.resourceSize(OFFHEAP, 32 * RESOURCE_SIZE).concurrency(4).versionLimit(2).storage(storage)).build();
  }
}
