 ...
 * @author cschanck
 **/
public class SovereignFRSStorage extends AbstractRestartabilityBasedStorage {
  public SovereignFRSStorage(PersistenceRoot root, SovereignBufferResource resource) {
    this(root,
         resource,
         AbstractRestartabilityBasedStorage.SOV_COMPACTION_RUN_INTERVAL_SECONDS,
         AbstractRestartabilityBasedStorage.SOV_COMPACTION_TRIGGER_THRESHOLD_PERCENTAGE,
         AbstractRestartabilityBasedStorage.SOV_COMPACTION_PERCENTAGE_AMOUNT);
  }

  public SovereignFRSStorage(PersistenceRoot root,
    ...
    super(root, resource,
          compactionRunIntervalSeconds,
          compactionTriggerThresholdPercentage,
          compactionPercentageAmount);
  }
  ...
  public SovereignRestartableBroker<?> makeBroker(SovereignDatasetImpl<?> impl) {
    return new FRSBroker(SovereignRestartableBroker.uuidToBuffer(impl.getUUID()), getDatasetStore(), impl);
  }

  @Override
  public <K extends Comparable<K>> Space<?, ?> makeSpace(SovereignRuntime<K> runtime) {
    return new FRSSpace(runtime);
  }

  @Override
  public boolean isCompatible(PersistentStorageType storageType) {
    return PersistentStorageType.FRS_PERMANENT_ID == storageType.getPermanentId();
  }
