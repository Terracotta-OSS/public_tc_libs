...
public class Float64FieldTest {
  ...
  public void testFloat64() {
    ...
    Float64Field bf = new Float64Field(10, 1);
    ...
    bf.put(accessor, 1.0d);
    Assert.assertThat(bf.get(accessor), is(1.0d));
    bf.put(accessor, 11.0d);
    Assert.assertThat(bf.get(accessor), is(11.0d));
  }
  ...
  public void testFloat64Array() {
    DataBuffer db = new SingleDataByteBuffer(ByteBuffer.allocate(1024));
    Accessor accessor = new Accessor(db, 0);
    Float64Field bf = new Float64Field(10, 6);
    ...
    for (int i = 0; i < bf.getAllocationCount(); i++) {
      Assert.assertThat(bf.get(accessor, i), is((double) i));
    }
    ...
    Assert.assertThat(bf.get(accessor, 0), is((double) (0)));
    for (int i = 1; i < bf.getAllocationCount(); i++) {
      Assert.assertThat(bf.get(accessor, i), is((double) (i - 1)));
    }
    bf.move(accessor, 1, 0, bf.getAllocationCount() - 1);
    Assert.assertThat(bf.get(accessor, bf.getAllocationCount() - 1), is((double) (bf.getAllocationCount() - 1 - 1)));
