...
public class Float32FieldTest {
  ...
  public void testFloat32() {
    ...
    Float32Field bf = new Float32Field(10, 1);
    ...
    bf.put(accessor, 1.0f);
    Assert.assertThat(bf.get(accessor), is(1.0f));
    bf.put(accessor, 11.0f);
    Assert.assertThat(bf.get(accessor), is(11.0f));
  }
  ...
  public void testFloat32Array() {
    DataBuffer db = new SingleDataByteBuffer(ByteBuffer.allocate(1024));
    Accessor accessor = new Accessor(db, 0);
    Float32Field bf = new Float32Field(10, 6);
    ...
    for (int i = 0; i < bf.getAllocationCount(); i++) {
      Assert.assertThat(bf.get(accessor, i), is((float) i));
    }

    // test move
    for (int i = 0; i < bf.getAllocationCount(); i++) {
      bf.put(accessor, i, (float) i);
    }
    bf.move(accessor, 0, 1, bf.getAllocationCount() - 1);
    Assert.assertThat(bf.get(accessor, 0), is((float) (0)));
    for (int i = 1; i < bf.getAllocationCount(); i++) {
      Assert.assertThat(bf.get(accessor, i), is((float) (i - 1)));
    }
    bf.move(accessor, 1, 0, bf.getAllocationCount() - 1);
    Assert.assertThat(bf.get(accessor, bf.getAllocationCount() - 1), is((float) (bf.getAllocationCount() - 1 - 1)));
