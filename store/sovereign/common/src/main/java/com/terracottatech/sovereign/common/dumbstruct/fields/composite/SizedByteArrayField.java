...
/**
 * Created by cschanck on 10/31/2014.
 */
public interface SizedByteArrayField extends StructField {
  int getLength(Accessor a);

  byte[] getBytes(Accessor a);

  void getBytes(Accessor a, byte[] dest, int offset);

  void putBytes(Accessor a, byte[] barr);

  void putBytes(Accessor a, byte[] barr, int srcOffset, int length);
}
