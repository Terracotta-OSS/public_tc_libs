...
public class Multiply {
  ...
  public static class Int<T> extends ArithmeticOperation.Int<T> {
    ...
    public static <T> IntrinsicBuildableToIntFunction<T> multiply(IntrinsicBuildableToIntFunction<T> a, int b) {
      if (b == 1) {
        return a;
      } else {
        return new Multiply.Int<>(a, b);
      }
    }

    public Int(IntrinsicBuildableToIntFunction<T> function, int operand) {
      super(IntrinsicType.MULTIPLY_INT, "*", function, operand);
    }
  ...
  public static class Long<T> extends ArithmeticOperation.Long<T> {
    ...
    public static <T> IntrinsicBuildableToLongFunction<T> multiply(IntrinsicBuildableToLongFunction<T> a, long b) {
      if (b == 1) {
        return a;
      } else {
        return new Multiply.Long<>(a, b);
      }
    }

    public Long(IntrinsicBuildableToLongFunction<T> function, long operand) {
      super(IntrinsicType.MULTIPLY_LONG, "*", function, operand);
    }
  ...
  public static class Double<T> extends ArithmeticOperation.Double<T> {
    ...
    public static <T> IntrinsicBuildableToDoubleFunction<T> multiply(IntrinsicBuildableToDoubleFunction<T> a, double b) {
      if (b == 1) {
        return a;
      } else {
        return new Multiply.Double<>(a, b);
      }
    }

    public Double(IntrinsicBuildableToDoubleFunction<T> function, double operand) {
      super(IntrinsicType.MULTIPLY_DOUBLE, "*", function, operand);
    }
