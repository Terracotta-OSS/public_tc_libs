...
public class Divide {
  ...
  public static class Int<T> extends ArithmeticOperation.Int<T> {
    ...
    public static <T> IntrinsicBuildableToIntFunction<T> divide(IntrinsicBuildableToIntFunction<T> a, int b) {
      if (b == 1) {
        return a;
      } else {
        return new Divide.Int<>(a, b);
      }
    }

    public Int(IntrinsicBuildableToIntFunction<T> function, int operand) {
      super(IntrinsicType.DIVIDE_INT, "/", function, operand);
    }
  ...
  public static class Long<T> extends ArithmeticOperation.Long<T> {
    ...
    public static <T> IntrinsicBuildableToLongFunction<T> divide(IntrinsicBuildableToLongFunction<T> a, long b) {
      if (b == 1) {
        return a;
      } else {
        return new Divide.Long<>(a, b);
      }
    }

    public Long(IntrinsicBuildableToLongFunction<T> function, long operand) {
      super(IntrinsicType.DIVIDE_LONG, "/", function, operand);
    }
  ...
  public static class Double<T> extends ArithmeticOperation.Double<T> {
    ...
    public static <T> IntrinsicBuildableToDoubleFunction<T> divide(IntrinsicBuildableToDoubleFunction<T> a, double b) {
      if (b == 1) {
        return a;
      } else {
        return new Divide.Double<>(a, b);
      }
    }

    public Double(IntrinsicBuildableToDoubleFunction<T> function, double operand) {
      super(IntrinsicType.DIVIDE_DOUBLE, "/", function, operand);
    }
