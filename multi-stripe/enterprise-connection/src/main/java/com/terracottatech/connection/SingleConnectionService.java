...
/**
 * Concrete implementation of {@link ConnectionService} which can handle stripe connections and throw the enterprise-only
 * {@link HandshakeOrSecurityException}
 */
public class SingleConnectionService implements ConnectionService {
  private static final String CONNECTION_TYPE = "stripe";
  ...
  public final Connection connect(URI uri, Properties properties) throws ConnectionException {
    ConnectionService connectionService = new TerracottaConnectionService() {
      public boolean handlesURI(URI uri) {
        return SingleConnectionService.this.handlesURI(uri);
      }
    };
  ...
  public final Connection connect(Iterable<InetSocketAddress> servers, Properties properties) throws ConnectionException {
    ConnectionService connectionService = new TerracottaConnectionService() {
      public boolean handlesConnectionType(String connectionType) {
        return SingleConnectionService.this.handlesConnectionType(connectionType);
      }
    };
